  ...
  class Config
    ...
    def multisite_subdomains?
      @using_multisite_subdomains ||= begin
        laravel_sites.any? do |(_name, site)|
    ...
    def site_hosts
      @site_hosts ||= begin
        laravel_sites.flat_map { |(_name, site)| site['site_hosts'] }.tap do |hosts|
    ...
    def laravel_sites
      @laravel_sites ||= begin
        content['laravel_sites'].tap do |sites|
    ...
    def path
      File.join(@root_path, 'group_vars', 'development', 'laravel_sites.yml')
    end
